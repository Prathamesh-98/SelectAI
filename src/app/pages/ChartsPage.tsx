      ...
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <motion.div
            ...
            style={{ backgroundColor: d.color ?? '#3B82F6', minHeight: 2 }}
          />
    ...
    >
      ...
      <div>
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          <span className="text-[10px] font-mono text-zinc-700 bg-white/5 px-1.5 py-0.5 rounded flex-shrink-0 mt-0.5">
            {ch.datasetName}
          </span>
        </div>
        {ch.description && (
          <p className="text-[12px] text-zinc-600 leading-relaxed">{ch.description}</p>
        )}
      </div>

      {/* Chart */}
      <div className="bg-white/[0.02] rounded-xl p-4 border border-white/4">
        <MiniBar data={ch.data} />
      </div>

      {/* Footer */}
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-zinc-700">Saved {ch.createdAt}</span>
        <span className="text-[10px] font-semibold uppercase tracking-wider text-zinc-600 bg-white/5 px-2 py-1 rounded-full">
          {ch.type} chart
        </span>
      </div>
    </motion.div>
...
// ─── Component ────────────────────────────────────────────────────────────────
export function ChartsPage({ workspace }: Props) {
  return (
    <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-[22px] font-bold text-white tracking-tight">Charts</h1>
        <p className="text-[13px] text-zinc-500 mt-0.5">
          {workspace.savedCharts.length} chart{workspace.savedCharts.length !== 1 ? 's' : ''} saved in this workspace
        </p>
      </div>
      ...
      {workspace.savedCharts.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-[14px] font-semibold text-zinc-500 mb-1">No saved charts yet</p>
          <p className="text-[12px] text-zinc-700">Charts are saved automatically when you ask AI to visualise data</p>
        </div>
      )}
    </div>
  )
