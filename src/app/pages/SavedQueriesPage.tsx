    ...
    >
      ...
      <div>
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          <span className="text-[10px] font-mono text-zinc-700 flex-shrink-0 mt-0.5 bg-white/5 px-1.5 py-0.5 rounded">
            {q.datasetName}
          </span>
        </div>
        <p className="text-[11px] text-zinc-700">
          Saved {q.createdAt} · Run {q.runCount}× 
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 pt-1">
        <button
          ...
          onClick={handleRun}
          disabled={running}
          className={`flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12px] font-semibold transition-all duration-200 ${
            running
...
export function SavedQueriesPage({ workspace }: Props) {
  const [search, setSearch] = useState('')

  const filtered = workspace.savedQueries.filter(q =>
    q.title.toLowerCase().includes(search.toLowerCase()) ||
    q.datasetName.toLowerCase().includes(search.toLowerCase())
  )
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex-1">
          ...
          <p className="text-[13px] text-zinc-500 mt-0.5">
            {workspace.savedQueries.length} queries saved in this workspace
          </p>
        </div>
      </div>
      ...
      {filtered.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-[14px] font-semibold text-zinc-500 mb-1">
            {search ? `No queries match "${search}"` : 'No saved queries yet'}
          </p>
          <p className="text-[12px] text-zinc-700">
            {search ? 'Try a different search term' : 'Run a query from AI Chat and save it here'}
          </p>
        </div>
      )}
    </div>
